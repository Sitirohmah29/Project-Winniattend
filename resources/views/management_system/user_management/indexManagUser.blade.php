...
<head>
    ...
    <title>Management - User Management</title>
</head>
...
<body>
    <div class="dashboard flex flex-row max-w-screen">
        ...
        <div class="navbar fixed left-0 top-0 h-full flex flex-col justify-start bg-white shadow-lg px-6 py-8 w-58 z-10">
            ...
            <div class="nav-list flex flex-col gap-4">
                ...
                <div class="Attedance flex flex-row gap-2 items-center"
                    ...
                    <button class="text-gray-500 text-sm">Attedance Management</button>
                </div>
                ...
                <div class="management-user flex flex-row gap-2 items-center"
                    ...
                    <button class="text-blue-500 text-sm">Management User</button>
                </div>
        ...
        {{-- Content --}}
        <div class="content flex flex-col gap-6 ml-58 p-6 w-full overflow-y-auto">
            <!-- Header -->
            ...
            <div class="flex gap-4 items-center justify-between">
                <!-- Search Box -->
                <form method="GET" action="{{route('users.search')}}" class="flex items-center bg-white rounded-full shadow-md px-4 py-2 w-[450px]">
                        <i class="fa fa-search text-gray-500 mr-2"></i>
                        <input type="text" placeholder="Search" name="search" value="{{ request('search') }}"  class="w-full bg-transparent outline-none text-base italic text-gray-700" />
                </form>
            ...
            <div class="relative">
                ...
                <ul x-show="openRole" @click.away="openRole = false" class="absolute z-10 mt-2 w-full bg-white border rounded-md shadow-lg">
                    <li><a href="{{ route('users.search', array_merge(request()->except('role_id'), ['role_id' => ''])) }}" class="block px-4 py-2 hover:bg-gray-100">All Roles</a></li>
            ...
            <div class="relative">
                ...
                <ul x-show="openShift" @click.away="openShift = false" class="absolute z-10 mt-2 w-full bg-white border rounded-md shadow-lg">
                    <li><a href="{{ route('users.search', array_merge(request()->except('shift'), ['shift' => ''])) }}" class="block px-4 py-2 hover:bg-gray-100">All Shifts</a></li>
                    <li><a href="{{ route('users.search', array_merge(request()->except('shift'), ['shift' => 'shift-1'])) }}" class="block px-4 py-2 hover:bg-gray-100">Shift-1</a></li>
                    <li><a href="{{ route('users.search', array_merge(request()->except('shift'), ['shift' => 'shift-2'])) }}" class="block px-4 py-2 hover:bg-gray-100">Shift-2</a></li>
                    <li><a href="{{ route('users.search', array_merge(request()->except('shift'), ['shift' => 'shift-3'])) }}" class="block px-4 py-2 hover:bg-gray-100">Shift-3</a></li>
                </ul>
            </div>
            ...
            <div class="relative">
                <button type="button" @click="openStatus = !openStatus"
                    class="flex items-center bg-white rounded-full shadow-md px-4 py-2 w-[150px] justify-between">
                    <span class="italic text-gray-700">
                        @if(request('is_active') === '1') Active
                        @elseif(request('is_active') === '0') Inactive
                        @else All Status @endif
                    </span>
                    <i class="fa fa-chevron-down text-gray-500 text-sm"></i>
                </button>
                <ul x-show="openStatus" @click.away="openStatus = false" class="absolute z-10 mt-2 w-full bg-white border rounded-md shadow-lg">
                    <li><a href="{{ route('users.search', array_merge(request()->except('is_active'), ['is_active' => ''])) }}" class="block px-4 py-2 hover:bg-gray-100">All Status</a></li>
                    <li><a href="{{ route('users.search', array_merge(request()->except('is_active'), ['is_active' => '1'])) }}" class="block px-4 py-2 hover:bg-gray-100">Active</a></li>
                    <li><a href="{{ route('users.search', array_merge(request()->except('is_active'), ['is_active' => '0'])) }}" class="block px-4 py-2 hover:bg-gray-100">Inactive</a></li>
                </ul>
            </div>
                ...
                <!-- Add User Button -->
                <button onclick="showAddUserForm()" class="flex items-center justify-center bg-blue-500 text-white w-40 px-4 py-2 rounded-full shadow-md hover:bg-blue-600 transition-colors duration-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex flex-row gap-2 items-center">
                                        <!-- Tombol Edit -->
                                        <form action="{{ route('users.create', ['edit_id' => $user->id]) }}#addUserFormContainer" method="GET" style="display:inline;">
            ...
            <div class="ml-30 mb-10 max-w-3xl bg-white p-8 rounded-3xl shadow-xl"
            id="addUserFormContainer"
            style="{{ $isEdit || old() ? 'display: block;' : 'display: none;' }}">
                <h2 class="text-center text-3xl font-bold text-gray-800 mb-6">
                    ...
                    <form class="flex flex-col gap-4"
                        action="{{ $isEdit ? route('users.update', $editUser->id) : route('users.store') }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="fullname"
                                value="{{ old('fullname', $isEdit ? $editUser->fullname : '') }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="phone"
                                value="{{ old('phone', $isEdit ? $editUser->phone : '') }}"
                                class="w-full px-4 py-2 rounded-md bg-pink-100 focus:outline-none" required />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="address"
                                value="{{ old('address', $isEdit ? $editUser->address : '') }}"
                                class="w-full px-4 py-2 rounded-md bg-pink-100 focus:outline-none" required />
                        </div>
                        ...
                        <div class="mt-10 grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="role_id" class="w-full px-4 py-2 rounded-md bg-pink-100 focus:outline-none"
                                    required>
                            ...
                            <div>
                                ...
                                <select name="shift" class="w-full px-4 py-2 rounded-md bg-pink-100 focus:outline-none"
                                    required>
