...
@section('content')
    ...
    <div class="flex flex-row justify-between items-center">
        <div>
            <h2 class="text-2xl text-pink-600 italic font-semibold">
        ...
        <div class="rounded-full bg-black p-3" onclick="window.location.href='{{ url('/notificationWeb') }}'">
            <svg width="18" height="18" viewBox="0 0 26 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
            ...
            <div class="flex flex-col w-full gap-3">
                @foreach ($roles as $role)
                    <div class="flex flex-row w-full justify-between items-center">
                        ...
                        <span class="text-gray-500 text-sm">{{ $role->users_count }} / {{ $totalEmployees }}</span>
                    </div>
                @endforeach
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @foreach ($attendances as $attendance)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-gray-800 font-medium"> {{ $attendance->user->fullname ?? '-' }}</td>
